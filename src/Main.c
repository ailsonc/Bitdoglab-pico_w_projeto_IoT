    ...
    for (;;)
    {
        if (xQueueReceive(dataQueue, &receivedData, portMAX_DELAY) == pdTRUE)
        {
            ...
            if (cnt == 10)
            {
                ...
                // Formata a string antes de chamar display_status_msg
                snprintf(display_buffer, sizeof(display_buffer), "Media: %.2f C", avg); 
                display_status_msg(display_buffer);
...
{ 
    ...
    display_init(); 
    display_multiline("Inicializacao", NULL); 
}
